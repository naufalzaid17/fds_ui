...
const InputNumber = dynamic(() => import('antd').then((antd) => antd.Button),{csr: true})
...
import { GetScenarioGroupFraud, AddScenarioGroupFraud, EditScenarioGroupFraud, DeleteScenarioGroupFraud } from "@/services/ScenarioGroupServices/Services";

const ScenarioGroupFraudReaction = () => {
    ...
    const {dataFraud, handleReadFraud} = GetScenarioGroupFraud();
    const handleAddFraud = AddScenarioGroupFraud();
    const handleEditFraud = EditScenarioGroupFraud();
    const handleDeleteFraud = DeleteScenarioGroupFraud();
...
const handleEdit = async(e) => {
  ...
  try {
    ...
    await handleEditFraud(editFormData, setUpdateItemIdFraud, setLoading, handleReadFraud, isEditFraudCancel);
  } catch (error) {
    console.log(error);
  }
}
  ...
  return (
    <Layout>
        ...
        <div className='mt-5 md:mt-0'>
            <button onClick={showModalCreate} className='bg-secondary md:w-full w-36  hover:bg-opacity-90 text-white  md:px-10 py-2 rounded-lg font-medium border-[1px] border-secondary text-xs text-center'>
      ...
      <Modal open={isModalCreateFraudOpen} onOk={isCreateFraudOK} onCancel={isCreateFraudCancel} closable={true} width={800} footer={[
        ...
        <Button key="submit" type="primary" htmlType="submit" onClick={handleAdd}>
          ...
          <div className="text-center text-secondary font-bold text-xl font-poppins">
            <h1>Create Rule Group Reaction</h1>
          </div>
              ...
              <div className="text-md">
                  ...
                  <div className='flex gap-10 mt-5'>
                      <div className="mt-1 w-1/2">
                          ...
                          <select className="minimal w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" name='group' value={action
                          } onChange={(e)=> setAction(e.target.value)} id='group'>
                            <option value="0" selected className=''>Select Field</option>
      ...
      <form className='font-poppins mt-8 font-medium'>
        <div className="text-md">
            ...
            <div className='flex gap-10 mt-5'>
                <div className="mt-1 w-1/2">
                    <label className="">Action</label>
                    <select className="minimal w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" name='action' value={editFormData.action} onChange={(e) => setEditFormData({ ...editFormData, action: e.target.value })}>
                            <option value="0" selected className=''>Select Field</option>
                            <option value="Create Alert">CREATE_ALERT</option>
                            <option value="Set Response Code">SET_RESPCODE</option>
                            <option value="SMS Notification">SMS_NOTIFICATION</option>
...
export default ScenarioGroupFraudReaction