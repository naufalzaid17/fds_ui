...
import { GetData, AddContactSetup, EditContactSetup, DeleteContactSetup, GetNotificationType } from "@/services/ContactSetupServices/Services";
...
const ContactSetup = () => {
  ...
  const handleAddData = AddContactSetup();
  const handleEditData = EditContactSetup();
  const handleDeleteData = DeleteContactSetup();
  ...
  const handleAdd = async(e) => {
    ...
    try {
      ...
      console.log(dataNew);
      await handleAddData(dataNew, setNotificationType, setFirstName, setLastName, setContactValue, setIsModalOpen, handleRead)
    } catch (error) {
      console.log(error);
    }
  }
  ...
  const showModalDelete = (record) => {
    console.log(record.recipientId);
      setIsModalDeleteOpen(true);
      setDeleteItemId(record.recipientId);
  }
  ...
  const showModalEdit = (record) => {
    ...
    setUpdateItemId(record.recipientId)
  ...
  const handleEdit = async(e) => {
    ...
    try {
      ...
      await handleEditData(editFormData, updateItemId, setUpdateItemId, handleRead, setLoading, isEditCancel);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <Layout>
      <HeadTitle title="Contact Setup"/>
      <HeadContent pageTitle="Contact Setup" caption="Setup the contact"/>
                    ...
                    <div className="text-md">
                        ...
                        <div className='flex gap-10'>
                            <div className="mt-4 w-1/2">
                                ...
                                <input className=" w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={firstName} onChange={(e)=> setFirstName(e.target.value)} id='firstName'/>
                            </div>
                            <div className="mt-4 w-1/2">
                                <label className="">Last Name</label>
                                <input className=" w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={lastName} onChange={(e)=> setLastName(e.target.value)} id='lastName'/>
                            </div>
                        </div>
                        <div className="mt-4 mb-6">
                            <label className="">Contact</label>
                            <input className="w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={contactValue} onChange={(e)=> setContactValue(e.target.value)} id='contactValue'/>
                        </div>
                    </div>
                </form>
            </Modal>
                ...
                <div className="text-md">
                        <div className="mt-4">
                            ...
                            <select
                              ...
                              id='notificationType'
                            >
                              <option value="0">Add Notification Type</option>
                              <option value="2">Email</option>
                              <option value="1">SMS</option>
                            </select>
                        </div>
                        <div className='flex gap-10'>
                            <div className="mt-4 w-1/2">
                                <label className="">First Name</label>
                                <input className=" w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={editFormData.firstName} onChange={(e) => setEditFormData({ ...editFormData, firstName: e.target.value })} id='firstName'/>
                            </div>
                            <div className="mt-4 w-1/2">
                                <label className="">Last Name</label>
                                <input className=" w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={editFormData.lastName} onChange={(e) => setEditFormData({ ...editFormData, lastName: e.target.value })} id='lastName'/>
                            </div>
                        </div>
                        <div className="mt-4 mb-6">
                            <label className="">Contact</label>
                            <input className="w-full border-secondary border-[1px] mt-[11px] md:mt-2 rounded-lg py-2 px-2 focus:outline-green-700" value={editFormData.contactValue} onChange={(e) => setEditFormData({ ...editFormData, contactValue: e.target.value })} id='contactValue'/>
                        </div>
                    </div>
                </form>
            </Modal>
