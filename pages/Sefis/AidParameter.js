...
import { GetData, AddAidParameterData, UpdateAidParameterData, DeleteAidParameterData } from "@/services/AidParameterServices/Services";
const AidParameter = () => {
  ...
  const handleAddData = AddAidParameterData();
  const handleUpdateData = UpdateAidParameterData();
  const handleDeleteData = DeleteAidParameterData();
  // Modal Create
  ...
  return (
    <Layout>
      <HeadTitle title="Aid Parameter" />
      <HeadContent pageTitle="Aid Parameter" caption="List of application identifier" />
      <div className="flex flex-col gap-8 pt-6">
        <div className='flex justify-between'>
          <div className='flex flex-col md:flex-row'>
            ...
            <Modal open={isModalOpen} onOk={handleOk} onCancel={handleCancel} closable={true} width={800} footer={[
              ...
              <div className="text-center text-secondary font-bold text-lg font-poppins">
                <h1>Create Aid Parameter</h1>
              </div>
              <form className="font-poppins mt-6 font-medium" onSubmit={handleAddData}>
                <div className="text-sm">
                  <div className="mt-4">
                    <label className="">AID</label>
                    <input className=" w-full border-secondary border-[1px]  mt-[11px] md:mt-2 rounded py-2 px-1 focus:outline-green-600" value={aid} onChange={(e)=> setAid(e.target.value)} id='aid'/>
                  </div>
                  <div className="mt-4">
                    <label className="">Label</label>
                    <input className=" w-full border-secondary border-[1px]  mt-[11px] md:mt-2 rounded py-2 px-1 focus:outline-green-600" value={label} onChange={(e)=> setLabel(e.target.value)} id='label'/>
                  </div>
                </div>
              </form>
            </Modal>
          ...
          <div className='flex flex-col md:flex-row md:gap-5'>
            ...
            >
              <div className="text-center text-secondary font-bold text-lg font-poppins">
                <h1>Edit Aid Parameter</h1>
              </div>
              <form className="font-poppins mt-6 font-medium">
                <div className="text-sm">
                <div className="mt-4">
                    <label className="">AID</label>
                    <input className=" w-full border-secondary border-[1px]  mt-[11px] md:mt-2 rounded py-2 px-1 focus:outline-green-600" value={editFormData.aid} onChange={(e) => setEditFormData({ ...editFormData, aid: e.target.value })} id='aid'/>
                  </div>
                  <div className="mt-4">
                    <label className="">Label</label>
                    <input className=" w-full border-secondary border-[1px]  mt-[11px] md:mt-2 rounded py-2 px-1 focus:outline-green-600" value={editFormData.label} onChange={(e) => setEditFormData({ ...editFormData, label: e.target.value })} id='label'/>
                  </div>
                  </div>
              </form>
            </Modal>
