...
import WhiteListDetail from './WhiteListDetail/[id]';
import WhiteListFraudReaction from './WhiteListFraudReaction';
import { GetData, AddWhiteList, EditWhiteList, DeleteWhiteList, GetFraudListType } from '@/services/WhiteListServices/Services';
...
const WhiteList = () => {
  ...
  const handleAddWhiteList = AddWhiteList();
  const handleEditWhiteList = EditWhiteList();
  const handleDeleteWhiteList = DeleteWhiteList();
  ...
  const handleAdd = async(e) => {
    ...
    try {
      ...
      console.log(dataNew);
      await handleAddWhiteList(dataNew, setDateIn, setDateOut, setValue, setEntityType, setIdUserGroup, setReason, handleRead, setIsModalOpen)
    } catch (error) {
        console.log(error);
    }
  }
  ...
  const showModalDetail = (record) => {
    ...
    const id = record.id;
      router.push(`/List-Management/WhiteListDetail/${id}`);
  };
  ...
  const handleDelete = async(e) => {
    e.preventDefault();
    try {
      await handleDeleteWhiteList(deleteItemId, setDeleteItemId, setLoading, handleRead, isDeleteCancel)
    } catch (error) {
      console.log(error);
    }
  }
  ...
  const showModalFraudReaction = (record) => {
    setDetailItemId(record.id);
    const id = record.id;
    router.push(`/List-Management/WhiteListFraudReaction?id=${id}`);
  }
  ...
  const handleEdit = async(e) => {
    ...
    try {
      ...
      await handleEditWhiteList(editFormData, updateItemId, setUpdateItemId, handleRead, setLoading, isEditCancel);
    } catch (error) {
      console.log(error);
    }
  }
...
const columns = [
        ...
        <Menu
          onClick={(e) => 
            {if (e.key === 'edit') {
              ...
              <WhiteListFraudReaction id={record.id}/>
  ...
  return (
    <Layout>
        <HeadTitle title="White List"/>
        <HeadContent pageTitle="White List" caption="A white list present in an incoming transaction results in the transaction being automatically flagged as legal."/>
        ...
        <div className='flex justify-between'>
          ...
          <Modal open={isModalOpen} onOk={handleOk} onCancel={handleCancel} closable={true} width={800} footer={[
            ...
            {/* Modal Delete */}
            <Modal open={isModalDeleteOpen} onOk={handleDelete
            } onCancel={isDeleteCancel} closable={true} okText="OK">
