...
import { DetailWhiteList } from '@/services/WhiteListServices/Services';
...
const WhiteListDetail = () => {
    ...
    const {handleDetailWhiteList, dataDetail} = DetailWhiteList(id);

    useEffect(() => {
        if (id) {
          handleDetailWhiteList();
        }
      }, [id]);

    return (
      <Layout>
          <HeadTitle title="White List Detail"/>
          <HeadContent pageTitle="White List Detail" caption="More details about White List."/>
