...
import { GetData, LockCase, UnlockCase } from '@/services/AlertIssuerServices/Services';
...
const showModalDetail = (record) => {
  const id = record.ID;
  router.push(`/Monitoring/AlertIssuerDetail/${id}`);
};
...
const showModalConfiguration = (record) => {
  ...
  const hpan = record.HPAN;
  // console.log(record);
  router.push(`/Monitoring/AlertIssuerConfiguration?id=${id}&issuer=${issuer}&hpan=${hpan}`);
};
...
const columns = [
  ...
  return (
    <Layout>
      <HeadTitle title="Alert Issuer"/>
      <HeadContent pageTitle="Alert Issuer" caption="Alerts are used in analyzing a fraudulent authorization or transaction by an operator."/>
