...
import { GetDetailIssuer } from "@/services/AlertIssuerServices/Services";
...
const AlertIssuerDetail = () => {
  const [loading, setLoading] = useState(true);
  const router = useRouter();
  const { id } = router.query;
  const {dataDetail, handleDetail} = GetDetailIssuer(id, setLoading);
  ...
  return (
    <Layout>
      <HeadTitle title="Alert Issuer Detail" />
      <HeadContent pageTitle="Alert Issuer Detail" caption="More details about alert issuer" />
...
export default AlertIssuerDetail;
