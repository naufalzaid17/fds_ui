...
export function GetData(){
    ...
    const handleRead = async () => {
        const res = await axios.get(`http://${apiAddress}/whitelist/list`).then(
            res => {
                setLoading(false);
                setListData(
...
export function AddWhiteList(){
    const handleAdd = async(dataNew, setDateIn, setDateOut, setValue, setEntityType, setIdUserGroup, setReason, handleRead, setIsModalOpen) => {
        try {
            const res = await axios.post(`http://${apiAddress}/whitelist/add`, dataNew);
            setDateIn('');
            setDateOut('');
            setValue('');
            setEntityType('');
    ...
    const handledit = async(editFormData, updateItemId, setUpdateItemId, handleRead, setLoading, isEditCancel) => {
        try {
            ...
            const res = await axios.post(
                `http://${apiAddress}/whitelist/update`,
                editFormData,
                {
                  params: {
                    currentId: id,
...
export function DeleteWhiteList(){
    const handleDelete = async(deleteItemId, setDeleteItemId, handleRead, setLoading, isDeleteCancel) => {
        try {
            const res = await axios.delete(`http://${apiAddress}/whitelist/delete/${deleteItemId}`);
...
export function GetDataFraud(id){
    ...
    const handleReadFraud = async() => {
        const res = await axios.get(`http://${apiAddress}/fraudReaction/listByBindingIdAndBindingType?bindingId=${id}&bindingType=whitelist`).then(
            res => {
            setLoading(false);
            setDataFraud(
                res.data.responseData.map(row => ({
...
export function DetailWhiteList(id){
    const [dataDetail, setDataDetail] = useState([]);
    const [loading, setLoading] = useState(false);
    const handleDetailWhiteList = async() => {
        try {
            const res = await axios.get(`http://${apiAddress}/whitelist/find/${id}`);
    ...
    return {dataDetail, handleDetailWhiteList}
}